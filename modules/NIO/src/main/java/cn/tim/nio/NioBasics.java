...
public class NioBasics {
    ...
    /**
     * 文件系统io与磁盘io
     * 文件系统是更高层次的抽象，我们写的代码不直接跟磁盘打交道，文件系统定义了文件名，路径，文件，文件属性等抽象概念。
     * 文件的元数据描述了哪些块包含文件数据，数据在哪里结束，最后一次修改时间等。
     *
     * 分页技术执行IO过程
     * 1 确定请求文件数据分布在哪些页上（磁盘扇区组）。
     * 2 在内核空间分配足够数量的内存页
     * 3 在内存页与磁盘文件系统页之间建立映射
     * 4 为每一个内存页产生页错误
     * 5 虚拟内存系统获取到页错误，安排页面调入
     * 6 页面调入完成，文件系统对原始数据进行解析，获取文件内容和属性信息
     */
    public void io() {
